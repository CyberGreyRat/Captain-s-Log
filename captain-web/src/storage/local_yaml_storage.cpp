...
std::string upper(std::string s){std::transform(s.begin(),s.end(),s.begin(),[](unsigned char c){return static_cast<char>(std::toupper(c));});return s;}std::string lower(std::string s){std::transform(s.begin(),s.end(),s.begin(),[](unsigned char c){return static_cast<char>(std::tolower(c));});return s;}std::string trim(std::string s){auto a=s.find_first_not_of(" \t\r\n");if(a==std::string::npos)return{};auto b=s.find_last_not_of(" \t\r\n");return s.substr(a,b-a+1);}std::string unquote(std::string s){s=trim(s);if(s.size()>1&&s.front()=='"'&&s.back()=='"')s=s.substr(1,s.size()-2);return s;}std::string quote(const std::string&s){std::string o="\"";for(char c:s){if(c=='\\'||c=='"')o+='\\';o+=c;}return o+'"';}
...
LocalYamlStorage::LocalYamlStorage(std::filesystem::path p):project_(std::move(p)){}bool LocalYamlStorage::writable(const std::string&t)const noexcept{auto u=upper(t);return u=="USR"||u=="SYS"||u=="SRS"||u=="SEC"||u=="ARCH"||u=="LLD"||u=="TEST"||u=="UT"||u=="IT"||u=="ST"||u=="AT"||u=="BUG";}
std::vector<domain::Requirement>LocalYamlStorage::all()const{std::vector<domain::Requirement>v;auto root=project_/"reqs";if(!std::filesystem::exists(root))return v;for(const auto&e:std::filesystem::recursive_directory_iterator(root)){auto ext=e.path().extension().string();if(e.is_regular_file()&&(ext==".yaml"||ext==".yml")&&e.path().filename()!="captainslog.yml"){auto r=parse(e.path());if(r.authority!=domain::Authority::remote)v.push_back(std::move(r));}}return v;}std::optional<domain::Requirement>LocalYamlStorage::find(const std::string&u)const{for(auto&r:all())if(r.uid==upper(u))return r;return std::nullopt;}
