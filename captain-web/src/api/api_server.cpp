...
namespace captain::api
{
    namespace
    {
        ...
        [[maybe_unused]] void write_events(const std::filesystem::path &path, const json &events)
        {
            ...
            if (!output)
                throw std::runtime_error("Cannot write web audit history.");
            ...
            if (error)
                throw std::runtime_error("Cannot replace web audit history.");
        }
    }
    ApiServer::ApiServer(
    ...
    void ApiServer::routes()
    {
        ...
        server_.Get("/favicon.ico", [this](const auto &, auto &response) {
    const auto content = read_binary(web_ / "favicon.ico");
    if (content.empty()) {
        response.status = 404;
        return;
    }

    response.set_content(content, "image/x-icon");
    response.set_header("Cache-Control", "no-cache"); });
        ...
        server_.Get("/api/v1/projects", [this](const auto &, auto &response)
                    { reply(response, {{"active_project", project_id_},
        ...
        server_.Get("/api/v1/history", [this](const auto &, auto &response)
                    { reply(response, {{"captain_log", read_binary(project_ / "reqs" / "captainslog.yml")},
        ...
        server_.Post(R"(/api/v1/requirements/([A-Za-z0-9_-]+)/archive)",
                     [this](const auto &request, auto &response)
                     {
                         try
                         {
                             ...
                             if (reason.empty())
                                 throw std::runtime_error("Archivierungsgrund ist erforderlich.");
                             auto values = repository_.all();
                             auto found = std::find_if(values.begin(), values.end(),
                                                       [&](const auto &item)
                                                       { return item.uid == uid; });
                             if (found == values.end())
                                 throw std::runtime_error("UID nicht gefunden.");
                     ...
                     [this](const auto &request, auto &response)
                     {
                         try
                         {
                             ...
                             auto found = std::find_if(values.begin(), values.end(),
                                                       ...
                                                       { return item.id == id; });
                             if (found == values.end())
                                 throw std::runtime_error("Dokumentlink nicht gefunden.");
                             if (found->kind.rfind("archived:", 0) != 0)
                                 found->kind = "archived:" + found->kind;
        ...
        server_.Post(R"(/api/v1/documents/([A-Za-z0-9_-]+)/restore)",
                     [this](const auto &request, auto &response)
                     {
                         try
                         {
                             ...
                             if (found->kind.rfind(prefix, 0) == 0)
                                 found->kind = found->kind.substr(prefix.size());
                             found->created_by = user();
                             const auto saved = repository_.update_file(*found);
