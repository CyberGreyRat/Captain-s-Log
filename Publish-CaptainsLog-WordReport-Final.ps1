...
try {
    Write-Host "[word 1/14] Starting Microsoft Word..."; $word=New-Object -ComObject Word.Application
    $word.Visible=$false;$word.DisplayAlerts=0;$word.ScreenUpdating=$false;$word.Options.CheckSpellingAsYouType=$false;$word.Options.CheckGrammarAsYouType=$false;$word.Options.SaveInterval=0;$word.Options.Pagination=$false
    Write-Host "[word 2/14] Creating document..."; $document=$word.Documents.Add()
