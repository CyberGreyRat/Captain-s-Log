...
function Read-RequirementFile {
    ...
    foreach ($raw in [System.IO.File]::ReadAllLines($Path, [System.Text.Encoding]::UTF8)) {
        ...
        if ($raw -match '^([A-Za-z0-9_]+):\s*(.*)$') {
            ...
            if ($key -in @('integration_template','history','implementations','test')) { $section = $key; continue }
...
try {
    ...
    try {
        ...
        # A logo failure must never prevent report creation. The text header
        # remains available even if Word refuses an image in the header range.
        try {
            $logoRange = $section.Headers.Item(1).Range.Duplicate
            $logoRange.Collapse(1)
            $headerLogo = $logoRange.InlineShapes.AddPicture($logo, $false, $true)
            $headerLogo.LockAspectRatio = -1
            $headerLogo.Width = $word.CentimetersToPoints($Design.HeaderLogoWidthCm)
        }
        catch {
            Write-Warning "Header logo was skipped: $($_.Exception.Message)"
        }
    ...
    Add-Heading $selection 'Berichtsinhalt' 2
    foreach ($line in @(
        '1. Management Summary',
        '2. Traceability-Baum',
        '3. Traceability-Übersicht',
        '4. Klassifizierung und Risiken',
        '5. Anforderungen im Detail',
        '6. Test- und Verifikationsnachweise',
        '7. Audit- und Berichtsnachweise'
    )) { Add-BodyParagraph $selection $line 1 }
    ...
    Write-Host '[word 5/14] Traceability overview...'
    Add-Heading $selection '3. Traceability-Übersicht' 1
    Add-ChapterDescription $selection $ChapterDescriptions.Matrix
    foreach ($parentItem in $items) {
        foreach ($childUid in $parentItem.children) {
            $childType = if ($itemsByUid.ContainsKey($childUid)) { Get-TypeName $itemsByUid[$childUid].type } else { 'Fehlendes Objekt' }
            Add-KeyValuePairs $selection @(
                @('Parent', "$($parentItem.uid) - $(Get-TypeName $parentItem.type)"),
                @('Child', "$childUid - $childType"),
                @('Beziehung', "$(Get-TypeName $parentItem.type) → $childType")
            )
        }
    }
    ...
    foreach ($item in @($items | Where-Object { $_.type -in @('CLASS','RISK') })) {
        ...
        if ($item.attributes.Count) {
            Add-Heading $selection 'Fachliche Angaben' 3
            $rows = @()
    ...
    foreach ($item in @($items | Where-Object { $_.type -notin @('CLASS','RISK','UT','IT','ST','AT','TEST') })) {
        ...
        if ($item.attributes.Count) {
            Add-Heading $selection 'Typspezifische Angaben' 3
            $rows = @()
            foreach ($key in $item.attributes.Keys) { $rows += ,@((Get-FieldLabel $key), (Join-DisplayValue $item.attributes[$key])) }
            Add-KeyValuePairs $selection $rows
        }
